...
async def _chroma_upsert(chroma, docs: List):
    # Many Chroma bindings are synchronous. Wrap in coroutine if necessary.
    # Here we attempt to call upsert API; if it's sync, run in threadpool.
    try:
        # prefer an async method if available
        upsert = getattr(chroma, "upsert", None)
        if upsert is None:
            # fallback to client API: assume chroma._collection.add is available.
            # This is pseudocode and may need minor adjustment per Chroma client lib.
            def sync_add():
            ...
            # if upsert is coroutine
            if callable(upsert):
                ret = upsert(docs)  # could be coroutine or not
...
async def delete_documents_by_faq_id(faq_id: str):
    ...
    # Implementation depends on chroma's API: typically delete where metadata matches.
    try:
        # Try high-level delete API (pseudocode)
        delete_fn = getattr(chroma, "delete", None)
        if delete_fn:
            # some clients accept where clause
            maybe_coro = delete_fn(ids=None, where={"faq_id": faq_id})
            if hasattr(maybe_coro, "__await__"):
                await maybe_coro
            return {"status": "deleted", "faq_id": faq_id}
        else:
            # fallback: query ids then delete by ids
            coll = chroma
            def sync_query_delete():
                # this section might need to be adjusted per chroma client
